...
export const SummaryMetricCard: React.FC<SummaryMetricCardProps> = ({
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      className={`flex items-center justify-between gap-2 px-3 py-2 rounded-2xl border shadow-sm ${styles.card}`}
    >
      ...
      {hasChart && (
        ...
        <div className="w-[72px] h-[36px] shrink-0 overflow-hidden">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 2, right: 0, left: 0, bottom: 2 }}>
              ...
              <YAxis hide domain={sparkMax > 0 ? [0, sparkMax] : [-1, 0]} />
              <Area
                type="monotone"
                dataKey="value"
