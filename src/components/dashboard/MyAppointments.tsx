              ...
              {filteredAppointments.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((app, idx) => (
                <div key={app.appointment_id} className="p-4 hover:bg-gray-50/50 transition-colors">
                  ...
                  <div className="flex flex-col gap-2 mt-2 pt-2 border-t border-gray-50">
                    ...
                    {app.queue_bucket && (
                      <span className={`inline-flex items-center w-fit px-2 py-1 mt-1 rounded-md border text-[8px] font-black uppercase tracking-widest ${
                        app.queue_bucket === 'IN_PROGRESS' ? 'bg-primary-teal/10 text-primary-teal border-primary-teal/20' :
                    ...
                    >
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-col gap-1.5">
                          <StatusBadge status={app.status} />
                          {app.queue_bucket && (
                            <span className={`inline-flex items-center w-fit px-2 py-0.5 rounded-md border text-[8px] font-black uppercase tracking-widest ${
                              app.queue_bucket === 'IN_PROGRESS' ? 'bg-primary-teal/10 text-primary-teal border-primary-teal/20' :
                              app.queue_bucket === 'READY' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                              app.queue_bucket === 'CALLED' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                              'bg-gray-50 text-gray-500 border-gray-100'
              ...
              <div className="bg-[#549E9E] px-8 py-6 flex justify-between items-start text-white shrink-0">
                <div>
                  <h2 className="text-2xl font-black tracking-widest uppercase mb-1">
                    {t('consultation_modal.title', 'PATIENT CONSULTATION (COMPLETED)')}
                  </h2>
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-white/90">
                    {/* Fallback to 'YOU' if user name isn't directly available in app, though user context usually has it. */}
                    {t('dashboard.menu.profile')} • {t('consultation_modal.token', 'TOKEN')} #{selectedPrescription.display_token_display || selectedPrescription.token_number} • {selectedPrescription.treatment_name}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="border border-gray-100 p-4 bg-gray-50/50">
                    <div className="flex items-center gap-2 text-gray-400 mb-2">
                      <User size={14} />
                      <span className="text-[10px] font-black uppercase tracking-widest">{t('consultation_modal.patient', 'PATIENT')}</span>
                    </div>
                    <div className="font-bold text-gray-800">
                      {selectedPrescription.booked_for_type === 'FAMILY_MEMBER' 
                        ? `${selectedPrescription.family_member_full_name} (${selectedPrescription.family_member_relationship})`
                        : (user?.name || 'Self')}
                    </div>
                  </div>
                  <div className="border border-gray-100 p-4 bg-gray-50/50">
                    <div className="flex items-center gap-2 text-gray-400 mb-2">
                      <User size={14} />
                      <span className="text-[10px] font-black uppercase tracking-widest">{t('consultation_modal.age_gender', 'AGE / GENDER')}</span>
                    </div>
                  ...
                  <div className="border border-gray-100 p-4 bg-gray-50/50">
                    <div className="flex items-center gap-2 text-gray-400 mb-2">
                      <Phone size={14} />
                      <span className="text-[10px] font-black uppercase tracking-widest">{t('consultation_modal.mobile', 'MOBILE')}</span>
                    </div>
                    <div className="font-bold text-gray-800">{user?.phone || 'N/A'}</div>
                  </div>
                  <div className="border border-gray-100 p-4 bg-gray-50/50">
                    <div className="flex items-center gap-2 text-gray-400 mb-2">
                      <MapPin size={14} />
                      <span className="text-[10px] font-black uppercase tracking-widest">{t('consultation_modal.branch', 'BRANCH')}</span>
                    </div>
                    <div className="font-bold text-gray-800">{selectedPrescription.branch_name}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <div className="flex items-center gap-2 text-[#549E9E] mb-3">
                        <FileText size={16} />
                        <span className="text-[10px] font-black uppercase tracking-widest">{t('consultation_modal.treatment_advice', 'TREATMENT ADVICE')}</span>
                      </div>
                      <div className="bg-gray-50/50 border border-gray-100 p-5 min-h-[150px]">
                        <p className="text-sm font-medium text-gray-700 whitespace-pre-wrap">{selectedPrescription.prescription.treatment_advice}</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {[7, 15, 30].map(days => (
                          <div 
                            key={days}
                            className={`flex-1 py-3 rounded-xl text-center text-[10px] font-black uppercase tracking-widest border-2 transition-colors ${
                              selectedPrescription.prescription.medication_duration_days === days 
